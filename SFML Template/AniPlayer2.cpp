...
void AniPlayer2::Update(float dt)
{
	...
	SetScale(currentClipInfo->flipX ? sf::Vector2f{ -1.f, 1.0f } : sf::Vector2f { 1.f, 1.f });

	const auto& clipId = (direction.x != 0.f || direction.y != 0.f) ? currentClipInfo->move :
		currentClipInfo->idle;
